...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  final Set<String> _readQueue = {};

  void _markNotificationsAsRead(List<QueryDocumentSnapshot> docs) async {
    final batch = _firestore.batch();

    for (var doc in docs) {
      if (doc['isRead'] == false && !_readQueue.contains(doc.id)) {
        batch.update(doc.reference, {'isRead': true});
        _readQueue.add(doc.id);
      }
    }

    if (batch != null) {
      // Add a small delay so user can see the notification as bold
